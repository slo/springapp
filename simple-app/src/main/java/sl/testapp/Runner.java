...
public class Runner {
    ...
    public static void main(final String[] args) {
        ...
        long delay = 1000L;
        producer(gateway, randomGen, (long)(delay*1.0));

        reader(provider, (long)(delay*1.0));
    ...
    private static void reader(ProviderSystem providerSystem, long delay) {
        TimerTask taskRead = new TimerTask() {
            @Override
            public void run() {
                LOGGER.debug("Received: " + providerSystem.receive());
                LOGGER.debug("Current size: " + providerSystem.size());
                
            }
        };
        Timer tr = new Timer("Reader");
        tr.schedule(taskRead, 0, delay);
    }
